...
- [Overview](#overview)
- [Organization Prerequisites](#organization-prerequisites)
...
This section describes the organizational readiness required before installing DPN nodes.

The following conditions must be met:

- Organization accreditations submitted and required agreements signed with Data Sharing Mechanism (DSM)  
- Organization registration application verified and approved by DSM
...
- Organization has accepted the **Technology Sharing Agreement (TSA)**  
...
- **Mutual TLS (mTLS)** enforced for communication between organizations  
...
For **Bring Your Own (BYO) components**, organizations must follow recommended security scanning practices.
...
- DSI DSM services  
- Other participating organizations during data exchange

Required certificate setup:

- A **Certificate Signing Request (CSR)** generated by the organization with the correct **Common Name (CN)** and **Subject Alternative Names (SANs)**  
...
- The organization plans to deploy **DPN on Azure` cloud infrastructure**  
- The organization will use **Azure DevOps** for **Continuous Integration and Continuous Deployment (CI/CD)** into the Azure landing zone
